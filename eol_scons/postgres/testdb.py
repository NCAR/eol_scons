...
This module can be imported from within scons as part of the eol_scons
package, or it can be called directly as a script using the full path to
the script.

    import eol_scons.postgres.testdb as ptb

    python eol_scons/postgres/testdb.py

This module defines the PostgresTestDB and AircraftTestDB classes.  Objects
can be attached to a SCons environment and can serve as a proxy for a
postgres test database running in the background.  To run tests against a
live postgresql database, this provides an abstraction to control the
database before, during, and after running test commands.
...
class PostgresTestDB:
    ...
    def dump(self, host=None, user=None, db=None, path=None, env=None, args=None):
        ...
        if not path and db:
            path = db + ".sql"
        cmd = ["pg_dump", "-C", "--no-owner", "-v", "--format=p"]
        if args:
            cmd += args
        if host:
            cmd += ["-h", host]
...
def main():
    ...
    if pg.personality == "postgrestestdb":
        pg = PostgresTestDB()
    args = sys.argv
    op = None if len(args) < 2 else args[1]
    if op == "init":
        sqlfile = None
        if len(args) > 2:
            sqlfile = args[2]
    ...
    elif op == "dump" and len(args) == 3:
        pg.dump(path=args[2])
    elif op == "dump" and len(args) == 2:
        pg.dump()
    elif op == "env":
