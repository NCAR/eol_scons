...
_warn_skips = (os.path.dirname(eol_scons.__file__),
               os.path.dirname(SCons.__file__))
_warned = {}


def WarnDeprecated(msg):
    "Issue a deprecation warning with the given message."
    if msg not in _warned:
        _warned[msg] = True
        print('*** ' + msg)
        print("*** Set PYTHONWARNINGS=default to see all deprecated calls.")
    # skip_file_prefixes is not until 3.12, so stacklevel is set based on
    # trial-and-error.
    # warnings.warn(DeprecationWarning(msg), skip_file_prefixes=_warn_skips)
    warnings.warn(DeprecationWarning(msg), stacklevel=4)
...
def AppendDoxref(env, ref):
    "DOXREF support is deprecated, AppendDoxref() calls should be removed."
    WarnDeprecated(AppendDoxref.__doc__)
