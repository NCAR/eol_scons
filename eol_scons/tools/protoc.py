...
def ProtocEmitter(target, source, env):
    ...
    for src in source:
        commonprefix = os.path.commonprefix(
            [dirOfCallingSConscript.path, src.srcnode().path])
...
def generate(env):
    ...
    env[
        'PROTOCPYTHONCOM'] = '$PROTOC ${["-I%s"%x for x in PROTOCPROTOPATH]} $PROTOCFLAGS ${"--python_out="+PROTOCPYTHONOUTDIR} ${PROTOCFDSOUT and ("-o"+PROTOCFDSOUT) or ""} ${SOURCES}'
