...
@code
    ...
    after the file, such as OUTPUT_DIRECTORY.  This way the template Doxyfile generated by doxygen can
...
import eol_scons
from eol_scons.methods import WarnDeprecated
...
def Doxyfile_contents(target, source, env):
    ...
    The order of precedence is DOXYFILE_DICT, DOXYFILE_TEXT, and
    DOXYFILE_FILE.  In other words, parameter settings in DOXYFILE_DICT and
    then DOXYFILE_TEXT override all others.  A few parameters will always be
    enforced by the builder over the DOXYFILE_FILE by appending them after the
    file, such as OUTPUT_DIRECTORY.  This way the template Doxyfile generated
    by doxygen can still be used as a basis, but the builder can still control
    where the output gets placed.  If any of the builder settings really need
    to be overridden, such as to put output in unusual places, then those
    settings can be placed in DOXYFILE_TEXT or DOXYFILE_DICT.
...
def SetDoxref(env, name, tagfile, url):
    "DOXREF support is deprecated, SetDoxref() calls should be removed."
    WarnDeprecated(SetDoxref.__doc__)
