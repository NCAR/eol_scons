...
class ToolOsxDmgAppWarning(SCons.Warnings.Warning):
    pass
...
def create_dmg( target, source, env) :
    ...
    os.system( 'hdiutil create ' + srcfolders + ' -volname %s %s' % (volname, dmg) )
...
def generate(env):
    ...
    The target is the name of the disk image to be created. It may or may not contain the .dmg extension.
    
    The source(s) are the directories which will be included in the disk image.
