...
class _Automoc(object):
    ...
    def __call__(self, target, source, env):
        ...
        Debug(
            "%s: scanning [%s] to add targets to [%s]."
            % (
                self.objBuilderName,
        ...
        for obj in source:
            ...
            if not obj.has_builder():
                ...
                Debug(
                    "scons: qt4: '%s' seems to be a binary. Discarded."
                    % str(obj),
                    env,
                )
                continue
            ...
            if SCons.Util.splitext(str(cpp))[1] not in cxx_suffixes:
                Debug(
                    "scons: qt4: '%s' is not a C++ file. Discarded."
                    % str(cpp),
                    env,
                )
                # c or fortran source
            ...
            for h_ext in header_extensions:
                ...
                if h:
                    Debug(
                        "scons: qt4: Scanning '%s' (header of '%s')"
                        % (str(h), str(cpp)),
                        env,
                    )
            ...
            if h and q_object_search.search(h_contents):
                ...
                Debug(
                    "scons: qt4: found Q_OBJECT macro in '%s', "
                    "moc'ing to '%s'" % (str(h), str(moc_cpp)),
                    env,
                )
...
def _locateQt4Command(env, command):
    ...
    if qt4BinDir:
        ...
        result = reduce(
            lambda a, b: a or env.WhereIs(b, [qt4BinDir]), cmds, None
        )
...
def create_builders():
    ...
    tsbuilder = SCons.Builder.Builder(
        action="$QT4_LUPDATE $SOURCES -ts $TARGETS", multi=1
    )
    qmbuilder = SCons.Builder.Builder(
        action=[
            "$QT4_LRELEASE $SOURCE",
        ],
        src_suffix=".ts",
    ...
    # Resource builder
    qrcscanner = Scanner(
        name="qrcfile", function=_scanResources, argument=None, skeys=[".qrc"]
    )
    ...
    uic4builder = SCons.Builder.Builder(
        action="$QT4_UIC4CMD",
...
def generate(env):
    ...
    if env.get("QT_VERSION", 4) != 4:
        msg = str(
            "Cannot require qt4 tool after another version "
...
QT4DIR typically does not need to be specified.""".strip(),
                None,
...
QT4INCDIR=/usr/include/qt4.""".strip(),
                None,
                PathVariable.PathAccept,
            )
        )
    ...
    if "QT4DIR" in env:
        ...
        print()
        print(
            "For Windows, QT4DIR must be set "
    ...
    env["QT4_UIC4CMD"] = [
        SCons.Util.CLVar(
            "$QT4_UIC $QT4_UICDECLFLAGS -o ${TARGETS[0]} $SOURCE"
        ),
    ]
    ...
    env["QT4_MOCFROMCXXCMD"] = [
        SCons.Util.CLVar(
            "$QT4_MOC $QT4_MOCFROMCXXFLAGS -o ${TARGETS[0]} $SOURCE"
        ),
...
def _checkQtCore(env):
    ...
    if not hasQt:
        Debug(
            "QtCore/Qt header file not found. "
...
def enable_modules(self, modules, debug=False):
    ...
    self.LogDebug(
        "Entering qt4 enable_modules(%s) with sys.platform=%s..."
    ...
    if sys.platform.startswith("linux") or sys.platform == "msys":
        ...
        for module in modules:
            if self["QT4DIR"] == USE_PKG_CONFIG:
                ...
                if hdir == "":
                    prefix = pc.RunConfig(
                        self, "pkg-config --variable=prefix QtCore"
                    )
                    if prefix == "":
                        print(
                            "Unable to build Qt header dir for adding module "
                            + module
                        )
                        return False
                ...
                if pc.CheckConfig(self, "pkg-config --exists " + module):
                    ...
                    cflags = pc.RunConfig(
                        self, "pkg-config --cflags " + module
                    )
                    self.MergeFlags(cflags, unique=1)
                    libflags = pc.RunConfig(
                        self, "pkg-config --libs " + module
                    )
                    ...
                    Debug(
                        "qt4.enable_modules appended %s to CPPPATH"
                        % os.path.join(hdir, module),
                        self,
                    )
                ...
                Debug(
                    "enabling module %s with QT4DIR=%s"
                    % (module, self["QT4DIR"]),
                    self,
                )
            ...
            if module == "QtDesigner":
                self.AppendUnique(
                    QT4_MOCFROMHFLAGS=["-I", os.path.join(hdir, module)]
                )
            if module == "QtGui":
    ...
    if sys.platform == "darwin":
        ...
        for m in modules:
            self.AppendUnique(
                CPPPATH=[
                    "/usr/local/lib/" + m + ".framework/Headers",
                ]
            )
